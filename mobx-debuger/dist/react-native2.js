import { isObservable, toJS, untracked, isAction, runInAction, autorun } from "mobx";
...
function toPlain(input, depth = MAX_DEPTH, seen = /* @__PURE__ */ new WeakSet(), isRoot = true) {
  ...
  seen.add(input);
  if ((_a = isObservable) == null ? void 0 : _a(input)) {
    try {
...
function collectMethods(instance) {
  ...
  for (const [key, d] of allDescriptors.entries()) {
    ...
    if (typeof val === "function") {
      const kind = ((_a = isAction) == null ? void 0 : _a(val)) ? "action" : detectFlow(val) ? "flow" : "method";
...
async function invokeMethod(instance, name, args = []) {
  ...
  const isAct = (_a = isAction) == null ? void 0 : _a(fn);
    ...
    try {
      ...
      if ((desc == null ? void 0 : desc.get) && !desc.set) {
        if (trackComputed) {
          ...
          const { untracked: untracked2 } = require("mobx");
          out[k] = untracked2(
...
const connectStore$1 = (name, store) => storeRegistry.register(name, store);
let useMobXDevTools;
let connectStore;
const isDev = process.env.NODE_ENV !== "production";
if (isDev) {
  useMobXDevTools = require("./useMobXDevTools.js").useMobXDevTools;
  connectStore = connectStore$1;
} else {
...
export {
  sanitizeForJson as a,
  connectStore as c,
  storeRegistry as s,
  useMobXDevTools as u
};
